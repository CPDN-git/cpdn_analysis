...
def read_data(batch,diag,time_proc,subregion):
    ...
    files=glob.glob(ddir+str(batch)+"/"+hadGem_name+bfolder+"/*201706-201708.nc")
    mean_vals=[]
...
#        inputstr=' -selmon,7 -fldmean'+subreg+' -sub '+ifile+' '+ddir+'historical/tasmax/day/tasmax_1961-1990_max_07_ensmean_timmean.nc'
	inputstr=' -monmax -runmean,5 -fldmean '+subreg+' -selmon,7 '+ifile
...
#        vals=cdo.runmean(5,options = "-f nc",input=inputstr,returnCdf=True).variables[output[diag][0]][:]
	basefile=ddir+'historical/'+hadGem_name+'/day/'+hadGem_name+'_1987-2013_07_timmean.nc'
	vals=cdo.sub(options = "-f nc",input=inputstr+" "+basefile,returnCdf=True).variables[output[diag][0]][:]
        vals = vals.flatten()
...
def plot_figure(diag,time_proc,subr,errb,restore):
    ...
    plot_distribution_data(dataHist,dataNat,diag,fig)
    plot_return_time_data(dataHist,dataNat,errb,diag,fig)
    plt.tight_layout()
    fig.savefig("combined_figure_"+diag+"_HadGEM3_A.png",dpi=28.75*2)

def plot_distribution_data(dataHist,dataNat,diag,fig): 
    ...
    ax.plot([threshold2,threshold2],[0,ymax],'k--',linewidth=2.5,zorder=3,label="2016 event")
    ax.plot([threshold,threshold],[0,ymax],'k:',linewidth=2.5,zorder=3,label="2017 event")

    risk_ratio(dataHist,dataNat,"2017",threshold)
...
def risk_ratio(dataHist,dataNat,year,threshold):
    HistCount=sum(np.array(dataHist)>threshold)
    NatCount=sum(np.array(dataNat)>threshold)

    RiskRatio=float(HistCount)/float(NatCount)
    print year+" Risk ratio for 2017 event ",RiskRatio


def plot_data(data,cols,plabel,ax,errb,lab,diag):
...
bsn=1000)
    ...
    ax.semilogx([min(x_data_all),max(x_data_all)],[threshold2,threshold2],'k--',linewidth=2.5,label=lab[0],zorder=2)
    ax.semilogx([min(x_data_all),max(x_data_all)],[threshold,threshold],'k:',linewidth=2.5,label=lab[1],zorder=2)
...
def plot_return_time_data(dataHist,dataNat,errb,diag,fig): 
    ...
    plot_data(np.array(dataHist),cols[1],"Historical",ax,errb,[None,None],diag)
    plot_data(np.array(dataNat),cols[0],"HistoricalNat",ax,errb,["2016 event","2017 event"],diag)
